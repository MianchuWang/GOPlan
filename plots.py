        ...
        for current_path, directories, files in os.walk(directory):
            for file in files:
                if file.endswith('.csv'):
                    ...
                    if full_path.split('/')[2].split('_')[1] == env_name:
...
def plot_results(file_paths):
    ...
    for file_path in file_paths:
        ...
        algo = file_path.split('/')[2].split('_')[0]  # Extract algo name from file path

        if 'total steps' in data.columns and 'return (policy)' in data.columns:
    ...
    for i, (algo, data_list) in enumerate(algo_data.items()):
        ...
        for data in data_list:
            interpolated = np.interp(common_steps, data['total steps'], data['return (policy)'])
    ...
    title = file_path.split('/')[2].split('_')[1]
    plt.xlabel('Total Steps')
    plt.ylabel('return (policy)')
    plt.grid(linestyle=':')
...
def plot_main_experiments():
    envs = ['FetchPush', 'FetchPickAndPlace']
    for env in envs:
